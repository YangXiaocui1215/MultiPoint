...
from src.dataset_new_add_view_and_caption import FewShotDataset_AddCaption


from src.multimodal_models import BertForPromptFinetuning, RobertaForPromptFinetuning, resize_token_type_embeddings
from src.multimodal_models import FusionContrastiveResNetRobertaForPromptFinetuning, FusionContrastiveResNetBertForPromptFinetuning
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    template2: str = field(
        default=None,
        metadata={"help": "Template2 for fusion learning"}
    )
...
def main():
        ...
        with open(data_args.template_list_location) as fp:
            ...
            for line in Lines:
                ...
                if 'caption' in data_args.task_name:
                    ...
                    line = line.replace('*cls*', '*cls*<image>_0*sep+*')
            ...
            if data_args.template_path is not None:
                ...
                if data_args.template_id is not None:
                    ...
                    if model_args.add_image == True:
                        if 'caption' in data_args.task_name:
                            data_args.template = data_args.template.replace('*cls*', '*cls*<image>_0*is*caption_0*sep+*')
                        else:
        ...
        if data_args.gpt3_in_context_head or data_args.gpt3_in_context_tail: 
            ...
            for instance_id in range(data_args.gpt3_in_context_num):
                ...
                for sent_id in range(sent_num):
                    ...
                    # Replace caption_{id}
                    if 'caption' in data_args.task_name:
                        sub_template = sub_template.replace("caption_{}".format(sent_id), "caption_{}".format(sent_num + sent_num * instance_id + sent_id))
                # Replace mask
    ...
    if data_args.task_name == "mul_mvsa_single_fusion_add_caption" or data_args.task_name =="mul_mvsa_multiple_fusion_add_caption" or data_args.task_name =="mul_tumblr_fusion_add_caption":
        if 'prompt' in model_args.few_shot_type:
    ...
    # Create tokenizer
    if model_args.model_name_or_path == 'roberta-large' or model_args.tokenizer_name=='roberta-large':
    ...
    if data_args.task_name == "mul_mvsa_single_fusion_add_caption" or data_args.task_name =="mul_mvsa_multiple_fusion_add_caption" or data_args.task_name =="mul_tumblr_fusion_add_caption":
        print('!!!!!!!!!!!!!!!!the trainer is Trainer_Fusion_and_Caption!!!!!!!!!!!!!!!!!')
    ...
    lock_name = os.path.join('save_log', data_args.task_name+'_log.lock')
    log_name = os.path.join('save_log', data_args.task_name+'_log')
