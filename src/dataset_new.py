...
def tokenize_multipart_input_add_image(
    ...
    if prompt:
        ...
        *xx* represent variables:
        ...
        special_token_mapping = {
            'cls': tokenizer.cls_token_id, 'mask': tokenizer.mask_token_id, 'sep': tokenizer.sep_token_id, 'sep+': tokenizer.sep_token_id, IMAGE_TOKEN: I
        }
        template_list = template.split('*') # Get variable list in the template
        # print('****************************template_list*********************************')
...
class FewShotDataset_new(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False, num_image_tokens=2, add_image=False, image_transform=None):
        self.args = args
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            if self.args.template_list is not None: ###他并不是用所有的template in self.args.template_list, 而是每个样例使用的template可能是不同的。
