    ...
    if prompt:
        ...
        ****************************template_list*********************************
        ...
        for part_id, part in enumerate(template_list):
            ...
            if (part[:4] == 'sent' or part[1:5] == 'sent') and part != ' sentiment':
                # If this part is the sentence, limit the sentence length
                sent_id = int(part.split('_')[1])
                if sent_id == 0:
                    if first_sent_limit is not None:
...
class FewShotDataset_AddCaption(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False, num_image_tokens=2, add_image=False, image_transform=None):
        ...
        if mode != "train":
            ...
            for query_idx, context_indices, bootstrap_idx in self.example_idx:
                ...
                if args.task_name == "mul_mvsa_single_fusion_add_caption" or args.task_name =="mul_mvsa_multiple_fusion_add_caption" or self.args.task_name =="mul_tumblr_fusion_add_caption":
    ...
    def new_convert_fn(
        ...
        if not use_demo:
            ...
            if self.args.task_name == "mul_mvsa_single_fusion_add_caption" or self.args.task_name =="mul_mvsa_multiple_fusion_add_caption" or self.args.task_name =="mul_tumblr_fusion_add_caption":
