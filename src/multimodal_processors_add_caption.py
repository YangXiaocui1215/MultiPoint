...
class AddCaptionMVSAClassificationProcessor(DataProcessor):
    """
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = line['label']
            examples.append(MultimodalInputExample(guid=guid, text_a=text_a, image_path=image_path, image_caption=image_caption, label=label))
        return examples


class AddCaptionTumblrMultimodalClassificationProcessor(DataProcessor):
    """
    Data processor for MVSA multimoal(text+image) classification datasets (MVSA-S, MVSA-M).
    """
...
class AddCaptionTwitterMultimodalClassificationProcessor(DataProcessor):
    """
    Data processor for MVSA multimoal(text+image) classification datasets (MVSA-S, MVSA-M).
    """
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, line['id'])
            aspect = line['aspect']
            text_s = line['text'].replace('$t$', aspect)
            image_path = line['image']
            image_caption = line['caption']
            label = line['label']
            # import ipdb; ipdb.set_trace()
            # print("+++++++++++++text_a is {}+++++++++++++++++++".format(text_s))
            examples.append(MultimodalInputExample
                            (guid=guid, text_a=text_s, aspect=aspect, image_path=image_path, image_caption=image_caption, label=label))
        return examples
    

class AddCaptionMASADMultimodalClassificationProcessor(DataProcessor):
    """
    Data processor for MVSA multimoal(text+image) classification datasets (MVSA-S, MVSA-M).
    """
